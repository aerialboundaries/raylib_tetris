...
# Windows上なら win-native、WSL上なら linux を実行
all:
ifeq ($(IS_WIN),1)
	$(MAKE) win-native
else
...
# OS自動判別の run ターゲット
# Windows上で `make run` と打てば win-native をビルドして実行します
run:
ifeq ($(IS_WIN),1)
	$(MAKE) run-native
else
...
help:
	...
	@echo "  make win-native : Windowsネイティブ用ビルド"
	@echo "  make run-native : Windowsネイティブ用ビルド＆実行"
