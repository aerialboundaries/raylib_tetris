...
static bool beginPlay = true; // This var is only true at the begining of the
                              // game, used for the first matrix creations
      ...
      if (!lineToDelete) {
        ...
        } else // Piece falling
        {
          ...
          if (gravityMovementCounter >= gravitySpeed) {
            ...
            // Check if the piece has collided with another piece or with the
            // boundings

            ResolveFallingMovement(&detection, &pieceActive);

            // Check if we fullfilled a line and if so, erase the line and pull
            // down the the lines above
          ...
          if (lateralMovementCounter >= LATERAL_SPEED) {
            // Update the lateral movement and if success, reset the lateral
            // counter
...
void DrawGame(void)
{
  ...
  if (!gameOver) {
    ...
    offset.y = screenHeight / 2 - ((GRID_VERTICAL_SIZE - 1) * SQUARE_SIZE / 2) +
               SQUARE_SIZE * 2;
    ...
    DrawText(TextFormat("LINES:      %04i", lines), offset.x, offset.y + 20, 10,
             GRAY);
...
static bool Createpiece()
{
  ...
  // If the game is starting and you are going to create the first piece, we
  // create an extra one
  if (beginPlay) {
    GetRandompiece();
    beginPlay = false;
...
static bool ResolveLateralMovement()
{
  ...
  if (IsKeyDown(KEY_LEFT)) // Move left
  {
    ...
    for (int j = GRID_VERTICAL_SIZE - 2; j >= 0; j--) {
      for (int i = 1; i < GRID_HORIZONTAL_SIZE - 1; i++) {
        ...
        {
          // Check if we are touching the left wall or we have a full square at
          // the left
    ...
    if (!collision) {
      for (int j = GRID_VERTICAL_SIZE - 2; j >= 0; j--) {
        for (int i = 1; i < GRID_HORIZONTAL_SIZE - 1;
             i++) // We check the matrix from left to right
        {
  ...
  } else if (IsKeyDown(KEY_RIGHT)) // Move right
  {
...

    {
      for (int i = 1; i < GRID_HORIZONTAL_SIZE - 1; i++) {
        if (grid[i][j] == MOVING) {
          // Check if we are touching the right wall or we have a full square at
          // the right
    ...
    if (!collision) {
      for (int j = GRID_VERTICAL_SIZE - 2; j >= 0; j--) {
        for (int i = GRID_HORIZONTAL_SIZE - 1; i >= 1;
             i--) // We check the matrix from right to left
        {
